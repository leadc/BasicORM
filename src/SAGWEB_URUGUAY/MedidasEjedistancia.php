<?php
    ...
    class MedidasEjedistancia extends BORMObject implements BORMObjectInterface{
        ...
        function Save(){
            ...
            if($this->UpdateSQL(["CODINSPE = '$this->codinspe'", "CODVEHIC = '$this->codvehic'", "SUBSTR(FECHALTA,0,10) = SUBSTR(SYSDATE,0,10)"]) <= 0){
                if($this->fecha_alta == ''){
                    $this->fecha_alta = date_format(new \DateTime('now', new \DateTimeZone('America/Argentina/Buenos_Aires')), 'Y-m-d H:i:s');
                }
                $this->InsertSQL();
            }
         ...
         */
        function Refresh(){
            if($this->codinspe == -102){
                return $this->RefreshBy(["CODINSPE = ".$this->codinspe, "CODVEHIC = ".$this->codvehic, "SUBSTR(FECHALTA,0,10) = SUBSTR(SYSDATE,0,10)"]);
            }else{
